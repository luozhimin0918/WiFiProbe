        ...
        switch (requestCode){
            case 2001:
                if(resultCode==-1){
                    //同意
                    AppUpdateInfo info;
                    if (!CheckApkExist.checkApkExist(CustomerInfoActivity.this, "com.gelian.device")) {
                        info = new GLSUpdateInfo();
                        if (CheckApkExist.checkApkExist(CustomerInfoActivity.this, "com.gelian.shopun")) {
                            info.setAppNumber(1);
                        } else {
                            info.setAppNumber(2);
                        }
...
//        xAxis.setXOffset(10f);
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (info.getPackageName().equals("com.ums.wifiprobe")) {
                    ...
                    //店知了未有更新
                    Toast.makeText(CustomerInfoActivity.this, "应用市场未检查到" + info.getAppName() + "更新", Toast.LENGTH_LONG).show();
                }
            }
        });
    ...
    public void showDownloadDialog(final String title, final AppUpdateInfo info) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                info.showDownloadDialog(CustomerInfoActivity.this, info.getAppName() + title, new OnDialogCloseListener() {
                    @Override
                    public void onClick(Dialog dialog, boolean confirm) {
